...
test('appStore load/save enforces resident retention without changing store shape', () => {
  ...
  assert.deepEqual(Object.keys(persisted), ['listings', 'alerts', 'scans', 'rejections', 'targetedDiscoveryObservations', 'ownerIdentityReviews', 'ownerCompDrafts', 'settings']);
...
test('applyActiveListingRetentionToStore returns a compatible store copy without mutating input', () => {
  ...
  assert.deepEqual(Object.keys(result.store), ['listings', 'alerts', 'scans', 'rejections', 'targetedDiscoveryObservations', 'ownerIdentityReviews', 'ownerCompDrafts', 'settings']);
