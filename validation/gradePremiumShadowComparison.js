...
} = require('./gradePremiumSignalMigration');
const {
...
const GRADE_PREMIUM_SHADOW_COMPARISON_SCHEMA_VERSION = '1.0.0';
const GRADE_PREMIUM_SHADOW_COMPARISON_SOURCE = 'grade_premium_shadow_comparison';
...
const REQUIRED_GRADE_PREMIUM_SHADOW_COMPARISON_FIELDS = Object.freeze([
  'schemaVersion',
  'source',
  'comparisonId',
  'createdAt',
...
function sumSoldSupport(soldSupport = {}) {
  const support = asObject(soldSupport);
  return ['sameGradeCount', 'lowerGradeCount', 'higherGradeCount', 'rawCount']
    .reduce((total, field) => total + Number(firstDefined(support[field], 0)), 0);
}
...
function compareEvidence(nativeOutput = {}, canonicalSignal = {}) {
  ...
  const expectedTrueSold = sumSoldSupport(soldSupport);
  const mismatches = [];
  const comparisons = [
    {
      field: 'soldSupport',
      nativePath: 'soldSupport',
      shadowPath: 'canonicalSignal.evidenceBasis.details',
      status: normalizeComparisonStatus(stableEqual(soldSupport, asObject(evidenceBasis.details))),
      nativeValue: clone(soldSupport),
      shadowValue: clone(asObject(evidenceBasis.details))
    },
    {
      field: 'trueSoldCount',
      nativePath: 'soldSupport.sameGradeCount + lowerGradeCount + higherGradeCount + rawCount',
      shadowPath: 'canonicalSignal.evidenceBasis.trueSoldCount',
      status: normalizeComparisonStatus(stableEqual(expectedTrueSold, evidenceBasis.trueSoldCount), String(evidenceBasis.trueSoldCount) === String(expectedTrueSold)),
      nativeValue: expectedTrueSold,
      shadowValue: clone(evidenceBasis.trueSoldCount)
    },
    {
      field: 'activeContextCount',
      nativePath: 'soldSupport.activeContextCount',
      shadowPath: 'canonicalSignal.evidenceBasis.activeListingCount',
      status: normalizeComparisonStatus(stableEqual(firstDefined(soldSupport.activeContextCount, UNKNOWN_VALUE), evidenceBasis.activeListingCount)),
      nativeValue: firstDefined(soldSupport.activeContextCount, UNKNOWN_VALUE),
...
function compareConfidence(nativeOutput = {}, canonicalSignal = {}, alignment = {}) {
  const hasNativeConfidence = pathExists(nativeOutput, 'confidence') || pathExists(nativeOutput, 'confidenceLevel');
  ...
  const comparisons = [
    {
      ...
      shadowPath: 'canonicalSignal.confidence',
      status: hasNativeConfidence ? normalizeComparisonStatus(stableEqual(nativeOutput.confidence, confidence.value)) : 'semantic_match',
      nativeValue: hasNativeConfidence ? clone(nativeOutput.confidence) : UNKNOWN_VALUE,
      shadowValue: clone(confidence),
      semanticReason: hasNativeConfidence ? 'native_confidence_supplied' : 'native_grade_premium_has_no_calibrated_confidence'
    },
    {
      ...
      nativeValue: clone(nativeOutput.version),
      shadowValue: clone(alignment.confidenceAlignment)
    }
  ];

  if (hasNativeConfidence && !stableEqual(nativeOutput.confidence, confidence.value)) {
...
function compareStatus(nativeOutput = {}, canonicalSignal = {}, alignment = {}, report = {}) {
  ...
  const comparisons = [
    {
      field: 'premiumJustification',
      nativePath: 'premiumJustification',
      shadowPath: 'canonicalSignal.normalizedOutput.status',
      status: normalizeComparisonStatus(stableEqual(normalizeString(nativeOutput.premiumJustification), normalized.status)),
      nativeValue: normalizeString(nativeOutput.premiumJustification),
      shadowValue: clone(normalized.status)
    },
    {
      field: 'premiumRiskLevel',
      nativePath: 'premiumRiskLevel',
      shadowPath: 'canonicalSignal.normalizedOutput.riskLevel',
      status: normalizeComparisonStatus(stableEqual(normalizeString(nativeOutput.premiumRiskLevel), normalized.riskLevel)),
      nativeValue: normalizeString(nativeOutput.premiumRiskLevel),
  ...
  for (const comparison of comparisons) {
    if (!['exact_match', 'semantic_match'].includes(comparison.status)) {
      mismatches.push(buildMismatch('changed_status_value', comparison.field, 'Status representation differs between native and shadow wrapper.', comparison.nativeValue, comparison.shadowValue));
    }
  }
...
function compareMetadata(nativeOutput = {}, canonicalSignal = {}, migration = {}) {
  ...
  const comparisons = [
    ...
    },
    {
      ...
      status: producer.name === 'gradePremiumEngine' ? 'semantic_match' : 'mismatch',
      nativeValue: normalizeString(nativeOutput.source),
  ...
  for (const comparison of comparisons) {
    if (!['exact_match', 'semantic_match'].includes(comparison.status)) {
      mismatches.push(buildMismatch('changed_metadata_value', comparison.field, 'Metadata representation differs between native and shadow wrapper.', comparison.nativeValue, comparison.shadowValue));
    }
  }
...
function summarizeGradePremiumShadowComparison(comparison = {}) {
  return deepFreeze({
    schemaVersion: GRADE_PREMIUM_SHADOW_COMPARISON_SCHEMA_VERSION,
...
function buildGradePremiumShadowComparisonFingerprint(comparison = {}) {
  const projection = clone(comparison);
  delete projection.comparisonFingerprint;
  delete projection.gradePremiumShadowComparisonFingerprint;
  return buildFingerprintFromProjection(projection);
}

function compareGradePremiumNativeToShadow(input = {}, options = {}) {
  const migration = firstDefined(input.migration, input.gradePremiumMigration)
    ? clone(firstDefined(input.migration, input.gradePremiumMigration))
  ...
  const migrationValidation = validateGradePremiumMigration(migration);
  ...
  const core = {
    schemaVersion: GRADE_PREMIUM_SHADOW_COMPARISON_SCHEMA_VERSION,
    source: GRADE_PREMIUM_SHADOW_COMPARISON_SOURCE,
    comparisonId: normalizeString(firstDefined(input.comparisonId, options.comparisonId, `grade-premium-shadow-comparison:${migration.sourceOutputFingerprint}`)),
    ...
    metadata: {
      ...
      comparisonScope: 'grade_premium_native_to_phase_13_shadow'
    }
  };
  const withSummary = {
    ...core,
    summary: summarizeGradePremiumShadowComparison(core)
  };
  const prevalidated = {
    ...withSummary,
    comparisonFingerprint: buildGradePremiumShadowComparisonFingerprint(withSummary)
  };
  const withValidation = {
    ...withSummary,
    validation: validateGradePremiumShadowComparison(prevalidated)
  };
  return deepFreeze({
    ...withValidation,
    comparisonFingerprint: buildGradePremiumShadowComparisonFingerprint(withValidation)
  });
}

function validateGradePremiumShadowComparison(comparison = {}) {
  const errors = [];
  const warnings = [];
  const authorityViolations = [];
  const fingerprintViolations = [];
  const sourceReferenceViolations = [];
  const missing = REQUIRED_GRADE_PREMIUM_SHADOW_COMPARISON_FIELDS.filter((field) => {
  ...
  if (comparison.schemaVersion !== GRADE_PREMIUM_SHADOW_COMPARISON_SCHEMA_VERSION) errors.push(validationIssue('invalid_schema_version', 'schemaVersion must match Grade Premium Shadow Comparison schema.', 'schemaVersion'));
  if (comparison.source !== GRADE_PREMIUM_SHADOW_COMPARISON_SOURCE) errors.push(validationIssue('invalid_source', 'source must be grade_premium_shadow_comparison.', 'source'));
  ...
  if (artifacts.migration) {
    const migrationValidation = validateGradePremiumMigration(artifacts.migration);
    ...
    if (artifacts.migration.migrationFingerprint && buildGradePremiumMigrationFingerprint(artifacts.migration) !== artifacts.migration.migrationFingerprint) {
  ...
  if (comparison.comparisonFingerprint && buildGradePremiumShadowComparisonFingerprint(comparison) !== comparison.comparisonFingerprint) {
...
module.exports = {
  GRADE_PREMIUM_SHADOW_COMPARISON_SCHEMA_VERSION,
  GRADE_PREMIUM_SHADOW_COMPARISON_SOURCE,
  PARITY_STATUSES,
  REQUIRED_GRADE_PREMIUM_SHADOW_COMPARISON_FIELDS,
  buildGradePremiumShadowComparisonFingerprint,
  compareGradePremiumNativeToShadow,
  summarizeGradePremiumShadowComparison,
  validateGradePremiumShadowComparison
};
